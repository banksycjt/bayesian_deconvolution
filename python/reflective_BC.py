from numpy import array, flip, zeros, shape
...
def add_padding_reflective_BC(input_img, inference):
    ...
    size_x = shape(input_img)[0]
    size_y = shape(input_img)[1]
...
def apply_reflective_BC_object(object, intermediate_img, padding_size):
    ...
    size_x = shape(object)[0] - 2*padding_size
    size_y = shape(object)[1] - 2*padding_size
...
def apply_reflective_BC_shot(shot_noise_image, intermediate_img, padding_size):
    ...
    size_x = shape(shot_noise_image)[1] - 2*padding_size
    size_y = shape(shot_noise_image)[2] - 2*padding_size
